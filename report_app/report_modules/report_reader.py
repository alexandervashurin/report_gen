...
def method_docx(dir_y):
    def m_stringos(str_s):
        for i in str_s:
            for j in slova:
                if j in i:
                    doc2.add_paragraph(
                        "", style='ListBullet').add_run(i).bold = True
    ...
    for ii in range(dir_files.__len__() - 1):
        ...
        title = [
            i.text for i in document.paragraphs if i.runs and 'По состоянию' in i.text]
        ...
        # номер пункта и наименование электровоза
        ph = re.compile(r'\d\d.\s\w\w\d\w\s№\d\d\d')
        p1 = re.compile(r'\S')
